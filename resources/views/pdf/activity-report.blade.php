...
<div class="header">
    ...
    <div class="header-right">
        <div class="label">{{ __('Client') }}</div>
        <div class="value" style="font-weight:bold;">{{ $client->name }}</div>
        @if($client->address)
            @if($client->address['line_1'] ?? null)
                <div class="value" style="margin-bottom:2px;">{{ $client->address['line_1'] }}</div>
            @endif
            @if($client->address['line_2'] ?? null)
                <div class="value" style="margin-bottom:2px;">{{ $client->address['line_2'] }}</div>
            @endif
...
<table>
    ...
    @foreach($lines as $line)
        <tr>
            ...
            <td>{{ $line->summary ?? $line->description ?? '' }}</td>
        </tr>
    @endforeach
    <tr class="totals-row">
...
<div class="footer">
    ...
    <div class="signature-box">
        <div class="signature-label">{{ __('Customer signature') }}</div>
    </div>
</div>
</body>
</html>
